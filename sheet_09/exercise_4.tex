...
We have for every~$1 \leq p < \infty$ and~$k \geq 1$ that
\begin{align*}
...
\end{align*}
where we use for the second equality that the map~$\abs{\sin(2^{k+2} \pi x) - \sin(2^{k+1} \pi x)}^p$ is~\dash{$(1/2)$}{periodic} (because~$k+1 \geq 2$), and for the third equality the change of variables~$x \to x/2$.
...
We note that~$\supp(f_k) \subseteq [k-1,k+1]$ for every~$k$.
...
If~$p < 2$ then the sequence~$(f_k)_k$ is unbounded in~$\Lp^p(\Real^n)$ because
\[
  \norm{f_k}_p^p
  =
  k^{n(1-p/2)} \norm{\varphi}_p^p
